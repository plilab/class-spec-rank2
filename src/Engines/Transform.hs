-- A simple expression for transforming stuff in a program.
module Engines.Transform where

import GHC.Plugins

-- A class to describe how to transform b's given a transformation on a's
-- (that could be components of b's).
class FullTransform a b where
    fullTransform :: (a -> a) -> b -> b
    fullTransformM :: (Monad m) => (a -> m a) -> b -> m b

    -- Fully transforming till we reach a fixed point. Fixed point checks
    -- require a function to transform it to something that can actually
    -- be compared by equality (e.g., deBruijnize).
    fullTransformMTillFixedPoint :: (Monad m, Eq c) => (b -> c) -> (a -> m a) -> b -> m b
    fullTransformMTillFixedPoint cmp f x =
        do
            x' <- fullTransformM f x
            if cmp x == cmp x'
                then return x
...
-- Everything from here on out is incredibly straightforward, since we just
-- traverse down towards stuff that can be transformed, and transform them.
...
instance FullTransform CoreExpr (Type, CoreExpr) where
    ...
    fullTransformM f (l, r) = do
        r' <- fullTransformM f r
        return (l, r')
