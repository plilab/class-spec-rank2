...
-- function returns Nothing.where function
...
matchAppViewsToTargets lbvs in_scope_binds (FnAppView f _ args : xs) targets =
  let -- The very first step is to perform a binding-time analysis of the
   ...
   in res ++ matchAppViewsToTargets lbvs in_scope_binds xs targets
  where
...
transformRHS :: FnAppView -> [CoreBind] -> CoreM (CoreExpr, [CoreExpr])
...
mkNewIdentifier t = do
  ...
  let name =
        mkInternalName
          uniq1
          (mkLocalOcc uniq2 (mkVarOcc "sat_OptimizingSYB"))
