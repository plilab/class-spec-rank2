module Pass.TypeConstantFold (typeConstantFold) where
...
{- | The 'typeConstantFold' pass traverses through the entire program and searches
occurrences of 'sameTypeRep' that can be evaluated at compile time, and replaces
them with their evaluations.
-}
typeConstantFold :: Opts -> CoreToDo
typeConstantFold opts = CoreDoPluginPass "Type-Constant Folding" (partialEvalModGuts opts)

prt :: (Outputable a) => Opts -> a -> CoreM ()
prt opts = prtIf (show_type_fold opts)
prtS :: Opts -> String -> CoreM ()
prtS opts = prtSIf (show_type_fold opts)
...
partialEvalModGuts opts mod_guts = do
    prtS opts $ info "Running type-constant folding"
    -- First, get all the CoreBinds in the program
    let all_core_binds :: [CoreBind] = mg_binds mod_guts
    all_new_core_binds :: [CoreBind] <- fullTransformM (typeConstantFoldExpr opts) all_core_binds
...
{- This is the part that actually matters. Here, we are traversing
 ...
 - it is found, we perform type-constant folding based on the rules
 - presented above. -}

-- | Partially evaluate an 'Expr'.
typeConstantFoldExpr ::
    Opts ->
    -- | The expression to evaluate
    Expr Var ->
    -- | The updated expression after partial
    -- evaluation (if it happened)
    CoreM (Expr Var)
{- This is the most interesting case. -}
typeConstantFoldExpr opts expr@(App (App (App (App (App (App (Var f) (Type _)) _) (Type t1)) (Type t2)) _) _)
    | fn_name == target_fn = do
        prtS opts $ info "Target expression found"
        prt opts expr
        -- check if type applications are provably equivalent
        if provablyEquivalentTypes t1 t2
            then do
                ...
                prt opts new_expr
                return new_expr
            else -- check if type applications are provably distinct

                if provablyDistinctTypes t1 t2
                    then do
                        ...
                        return expr
  where
    fn_name = nameStableString $ varName f
    target_fn = "$base$Data.Typeable.Internal$sameTypeRep"
typeConstantFoldExpr _ e = return e
